...
def create_song():
    ...
    song = db.songs.find_one({"id": song_req["id"]})
    if song:
        return {
            "Message": f"song with id {song_req['id']} already present"
        }, 302
    insert_id = db.songs.insert_one(song_req)
